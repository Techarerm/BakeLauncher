...
class forge_install:
    ...
    def convert_forge_data_to_real(self, depends_data, full_libraries_path):
        ...
        for key, value in redo_data.items():
            ...
            if Status:
                print(f"Key : {key} | Old Value : {value} >>> New Value : {path}", color='lightgreen')
                full_path = os.path.join(full_libraries_path, path)
                full_path = os.path.normpath(full_path)
                redo_data[key] = full_path
            else:
                if key.endswith("_SHA"):
                    file_name = key.removesuffix("_SHA")
                    value = value.replace("'", "").replace("'", "")
                    sha_list.append(f"{file_name}#{value}")

        for sha_info in sha_list:
            file_name, sha = sha_info.split("#")
            print(f"File name : {file_name} | SHA : {sha}", color='purple')
    ...
    def get_forge_all_processors_class_name_and_args(self, profile_data):
        ...
        for data in processors_data:
            ...
            if sides is None or side_type == "client":
